...
struct Handler {
  ...
  Handler() { cublasCreate_v2(&h); }

  ~Handler() { cublasDestroy_v2(h); }
};
...
void timer(const std::string &tag, float flo, const std::function<void()> &gemm,
           int repeat = 10) {
  float total = 0.0f;
  ...
  cudaEventElapsedTime(&total, begin, end);
