...
cudaError_t cutlass_gemm(const int m, const int n, const int k,
  ...
  using cutlassGemm =
      cutlass::gemm::device::Gemm<scalar_t, columnMajor, scalar_t, columnMajor,
                                  scalar_t, columnMajor>;
  cutlassGemm op;
...
template <typename scalar_t>
void naive_mm(const int m, const int n, const int k, scalar_t *__restrict__ a,
              const int lda, scalar_t *__restrict__ b, const int ldb,
              scalar_t *__restrict__ c, const int ldc, const scalar_t alpha,
              const scalar_t beta) {
...
template <typename scalar_t>
__global__ void gemm_v1(const int m, const int n, const int k,
                        scalar_t *__restrict__ a, const int lda,
                        scalar_t *__restrict__ b, const int ldb,
                        scalar_t *__restrict__ c, const int ldc,
                        const scalar_t alpha, const scalar_t beta) {
...
template <typename scalar_t>
__global__ __launch_bounds__(1024) void gemm_v2_32x32(
    const int m, const int n, const int k, scalar_t *__restrict__ a,
    const int lda, scalar_t *__restrict__ b, const int ldb,
    scalar_t *__restrict__ c, const int ldc, const scalar_t alpha,
    const scalar_t beta) {
  auto bidx = blockIdx.x, bidy = blockIdx.y, tidx = threadIdx.x,
       tidy = threadIdx.y;
  ...
  c[IDX2C(tidx, tidy, ldc)] =
      alpha * accumulate + beta * c[IDX2C(tidx, tidy, ldc)];
}

template <typename scalar_t>
__global__ void gemm_v3_32x32(const int m, const int n, const int k,
                              scalar_t *__restrict__ a, const int lda,
                              scalar_t *__restrict__ b, const int ldb,
                              scalar_t *__restrict__ c, const int ldc,
                              const scalar_t alpha, const scalar_t beta) {
...
template <typename scalar_t>
__global__ void gemm_v4_32x32(const int m, const int n, const int k,
                              scalar_t *__restrict__ a, const int lda,
                              scalar_t *__restrict__ b, const int ldb,
                              scalar_t *__restrict__ c, const int ldc,
                              const scalar_t alpha, const scalar_t beta) {
...
__global__ __launch_bounds__(256) void gemm_v5_32x32(
    const int m, const int n, const int k, scalar_t *__restrict__ a,
    const int lda, scalar_t *__restrict__ b, const int ldb,
    scalar_t *__restrict__ c, const int ldc, const scalar_t alpha,
    const scalar_t beta) {
  ...
  for (int l = 0; l < k; l += 32) {
    ...
    if (l + 32 < k) {
      a += lda << 5;
      b += 32;
      loada = *(scalarN_t *)&(a[((idx * N) & 31) + ((idx * N) >> 5) * lda]);
  ...
  }
  {
    ...
    for (int i = 0; i < N; ++i)
      ((scalar_t *)&resC)[i] =
          alpha * ((scalar_t *)&ansc)[i] + beta * ((scalar_t *)&resC)[i];
    *devC = resC;
  }
}

template <typename scalar_t, typename scalarN_t>
__global__ __launch_bounds__(256) void gemm_v6_64x64(
    const int m, const int n, const int k, scalar_t *__restrict__ a,
    const int lda, scalar_t *__restrict__ b, const int ldb,
    scalar_t *__restrict__ c, const int ldc, const scalar_t alpha,
    const scalar_t beta) {
  ...
  auto bidx = blockIdx.x, bidy = blockIdx.y, tidx = threadIdx.x;
  auto Ntidx = N * tidx;
  auto xRange = (((int)bidx + 1) << 6) - m, yRange = (((int)bidy + 1) << 6) - n;
  if (xRange > 0) {
    a -= xRange;
    c -= xRange;
  }
  ...
  for (int x = 0; x < N; x++) {
    scalarN_t *devC = (scalarN_t *)&(
                  c[(tidx & 15) * 4 + ((tidx >> 4) * 4 + x) * ldc]),
              loadC = *devC;
    for (int y = 0; y < N; y++) {
...
template <typename scalar_t, typename scalarN_t>
__global__ __launch_bounds__(256) void gemm_v7_128x128(
    const int m, const int n, const int k, scalar_t *__restrict__ a,
    const int lda, scalar_t *__restrict__ b, const int ldb,
    scalar_t *__restrict__ c, const int ldc, const scalar_t alpha,
    const scalar_t beta) {
  constexpr auto N = sizeof(scalarN_t) / sizeof(scalar_t);
  auto tidx = threadIdx.x, bidx = blockIdx.x, bidy = blockIdx.y;
  auto xRange = (((int)bidx + 1) << 7) - m, yRange = (((int)bidy + 1) << 7) - n;
  auto x_a = tidx & 31, y_a = tidx >> 5, x_b = tidx & 1, y_b = tidx >> 1,
    ...
    for (int l = 0; l < 8; l++) {
      ...
      for (int x = 0; x < 2; x++) {
        for (int y = 0; y < 2; y++) {
          for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
              ((scalar_t *)&accumulate[x][y][i])[j] +=
                  ((scalar_t *)&tmpA[x])[j] * ((scalar_t *)&tmpB[y])[i];
            }
          }
        }
      }
  ...
  for (int x = 0; x < 2; x++) {
    for (int y = 0; y < 2; y++) {
      for (int i = 0; i < N; i++) {
        ...
        for (int j = 0; j < N; j++) {
          ((scalar_t *)&loadC)[j] =
              alpha * ((scalar_t *)&accumulate[x][y][i])[j] +
